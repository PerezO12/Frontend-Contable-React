...
export var CostCenterMovements = function (_a) {
    ...
    var pageSize = 20;
    var _d = useCostCenterMovements(costCenter.id), movements = _d.movements, totalCount = _d.totalCount, loading = _d.loading, error = _d.error, refetch = _d.refetch;
    var handleDateRangeChange = function (field) { return function (e) {
        var _a;
        var newDateRange = __assign(__assign({}, dateRange), (_a = {}, _a[field] = e.target.value, _a));
        setDateRange(newDateRange);
    }; };
    var handleApplyFilters = function () {
        var filters = __assign(__assign({}, dateRange), { skip: 0, limit: pageSize });
    ...
    var handlePageChange = function (direction) {
        ...
        var filters = __assign(__assign({}, dateRange), { skip: newPage * pageSize, limit: pageSize });
        refetch(costCenter.id, filters);
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-green-50 p-4 rounded-lg">
              ...
              <p className="text-lg font-semibold text-green-900">
                {formatCurrency(movements.reduce(function (sum, mov) { return sum + parseFloat(mov.debit_amount || '0'); }, 0))}
              </p>
            </div>
            <div className="bg-red-50 p-4 rounded-lg">
              <p className="text-sm text-red-600">Créditos</p>
              <p className="text-lg font-semibold text-red-900">
                {formatCurrency(movements.reduce(function (sum, mov) { return sum + parseFloat(mov.credit_amount || '0'); }, 0))}
              </p>
            </div>
          </div>
        </div>
      ...
      <Card>
        <div className="card-body">
          {loading ? (<div className="text-center py-8">
              ...
              <Button onClick={function () { return refetch(costCenter.id); }}>
                Reintentar
              </Button>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {movements.map(function (movement) { return (<tr key={movement.id} className="hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 text-sm text-right font-mono">
                          {movement.debit_amount && parseFloat(movement.debit_amount) > 0 ? (<span className="text-green-600">
                              {formatCurrency(parseFloat(movement.debit_amount))}
                            </span>) : (<span className="text-gray-400">-</span>)}
                        </td>
                        <td className="py-3 px-4 text-sm text-right font-mono">
                          {movement.credit_amount && parseFloat(movement.credit_amount) > 0 ? (<span className="text-red-600">
              ...
              {totalPages > 1 && (<div className="flex items-center justify-between mt-6 pt-4 border-t">
                  ...
                  <div className="flex space-x-2">
                    <Button variant="secondary" size="sm" onClick={function () { return handlePageChange('prev'); }} disabled={currentPage === 0 || loading}>
                      Anterior
                    </Button>
                    <span className="px-3 py-1 text-sm text-gray-600">
                      Página {currentPage + 1} de {totalPages}
                    </span>
                    <Button variant="secondary" size="sm" onClick={function () { return handlePageChange('next'); }} disabled={currentPage >= totalPages - 1 || loading}>
                      Siguiente
                    </Button>
                  </div>
                </div>)}
