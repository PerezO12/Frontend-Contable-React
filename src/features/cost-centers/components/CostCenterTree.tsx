...
interface TreeNodeProps {
  costCenter: CostCenterTreeType;
  level: number;
  onCostCenterSelect?: (costCenter: CostCenterTreeType) => void;
  onCreateChild?: (parent: CostCenterTreeType) => void;
  expanded: Set<string>;
  onToggleExpand: (costCenterId: string) => void;
}
...
const TreeNode: React.FC<TreeNodeProps> = ({
  ...
  const isExpanded = expanded.has(costCenter.id);
    ...
    <div>
      <div 
        className={`
          ...
          ${onCostCenterSelect ? 'cursor-pointer' : ''}
          ${getLevelColor(costCenter.level)} border-current
        `}
      ...
      >
        ...
        <div className="flex-1 flex items-center justify-between min-w-0 ml-2">
          <div className="flex items-center space-x-3 min-w-0">
            ...
            {!costCenter.allows_direct_assignment && (
          ...
          <div className="flex items-center space-x-3">
            {onCreateChild && costCenter.allows_direct_assignment && (
              <Button
                size="sm"
                variant="secondary"
                onClick={(e) => {
...
export const CostCenterTreeComponent: React.FC<CostCenterTreeProps> = ({
  onCostCenterSelect,
  onCreateChild,
  activeOnly = true
}) => {
  ...
  const handleToggleExpand = (costCenterId: string) => {
    setExpanded(prev => {
  ...
  const expandAll = () => {
    const getAllCostCenterIds = (costCenters: CostCenterTreeType[]): string[] => {
      return costCenters.reduce((ids: string[], costCenter) => {
        ids.push(costCenter.id);
        if (costCenter.children) {
          ids.push(...getAllCostCenterIds(costCenter.children));
        }
        return ids;
      }, []);
    };
        ...
        {loading ? (
          ...
          <div className="max-h-96 overflow-y-auto border border-gray-200 rounded-lg">
            ...
            <div className="divide-y divide-gray-100">
              {costCenterTree.map((costCenter) => (
                <div key={costCenter.id} className="group">
                  <TreeNode
