...
export var SimpleExportControls = function (_a) {
    ...
    var handleExport = function () { return __awaiter(void 0, void 0, void 0, function () {
        var blob, timestamp, fileName, error_1, errorMessage;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    console.log('🚀 Iniciando exportación...', { selectedCostCenterIds: selectedCostCenterIds, costCenterCount: costCenterCount, exportFormat: exportFormat });
                ...
                case 1:
                    ...
                    return [4 /*yield*/, CostCenterService.exportCostCenters(selectedCostCenterIds, exportFormat)];
                case 2:
                    blob = _a.sent();
                    timestamp = new Date().toISOString().slice(0, 19).replace(/[:-]/g, '');
                    fileName = "centros_costo_".concat(costCenterCount, "_registros_").concat(timestamp, ".").concat(exportFormat);
                    console.log('💾 Iniciando descarga del archivo:', fileName);
                    // Descargar archivo
                    ExportService.downloadBlob(blob, fileName);
                    success("\u2705 Se exportaron ".concat(costCenterCount, " centro").concat(costCenterCount === 1 ? '' : 's', " de costo exitosamente en formato ").concat(exportFormat.toUpperCase()));
                    return [3 /*break*/, 5];
                case 3:
                    error_1 = _a.sent();
                    console.error('❌ Error al exportar centros de costo:', error_1);
                    errorMessage = error_1 instanceof Error ? error_1.message : 'Error desconocido al exportar';
                    showError("Error al exportar: ".concat(errorMessage));
                    return [3 /*break*/, 5];
                case 4:
    ...
    return (<div className="flex items-center space-x-3">
      ...
      {/* Botón de exportar */}
      <Button onClick={handleExport} disabled={isExporting || selectedCostCenterIds.length === 0} variant="primary" className="flex items-center space-x-2">
        {isExporting ? (<>
            <Spinner size="sm"/>
