import type { WizardState, StepperStep } from '../types';
...
export function ImportStepper({
  ...
  const getStepStatus = (step: StepperStep, index: number): StepperStep['status'] => {
    if (index < currentIndex) {
      return isStepValid(step.id as WizardState['currentStep']) ? 'completed' : 'error';
    }
    if (index === currentIndex) {
      return 'current';
    }
  ...
  const canClickStep = (step: StepperStep, index: number): boolean => {
    if (isLoading) return false;
    if (index === currentIndex) return false;
    if (index > currentIndex) return false;
    return isStepValid(step.id as WizardState['currentStep']);
  };  return (
    <div className="w-full py-6" style={{ lineHeight: '1.5' }}>
            ...
            return (
              ...
              >
                ...
                {index !== steps.length - 1 && (
                  <div
                    className="absolute top-4 left-1/2 w-full h-0.5 transform translate-x-1/2 z-0"
                    aria-hidden="true"
                  >
                    <div
                ...
                <button
                  type="button"
                  onClick={() => isClickable && onStepClick(step.id as WizardState['currentStep'])}
                  disabled={!isClickable}
                  className={`relative z-10 flex h-8 w-8 items-center justify-center rounded-full mb-3 ${
                    status === 'current'
