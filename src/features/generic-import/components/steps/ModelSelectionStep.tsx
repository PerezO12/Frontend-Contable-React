...
export function ModelSelectionStep({
  ...
  onModelSelect,
  isLoading,
}: ModelSelectionStepProps) {  return (
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {availableModels.map((modelName) => (
          <button
            ...
            disabled={isLoading}            className={`relative rounded-lg border p-4 flex flex-col items-start text-left hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="text-blue-700 ml-1">
                {modelMetadata.fields.filter(f => f.is_required).length}
              </span>
            </div>
          </div>

          {modelMetadata.business_key_fields.length > 0 && (
