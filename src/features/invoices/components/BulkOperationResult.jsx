...
export function BulkOperationResultDisplay(_a) {
    var result = _a.result, operation = _a.operation, onClose = _a.onClose;
    var getOperationLabel = function (op) {
        switch (op) {
    ...
    var successRate = result.total_requested > 0
    ...
    return (<Card className="p-6">
      <div className="space-y-6">
        ...
        <div className="w-full bg-gray-200 rounded-full h-3">
          <div className="bg-green-500 h-3 rounded-full transition-all duration-500" style={{ width: "".concat(successRate, "%") }}></div>
        </div>

        {/* Facturas exitosas */}
        {result.successful > 0 && (<div>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {result.successful_ids.map(function (id) { return (<Badge key={id} color="green" variant="subtle" className="mr-2">
                  {id}
                </Badge>); })}
            </div>
          </div>)}
        ...
        {result.failed > 0 && (<div>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {result.failed_items.map(function (item) { return (<div key={item.id} className="p-3 bg-red-50 rounded">
        ...
        {result.skipped > 0 && (<div>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {result.skipped_items.map(function (item) { return (<div key={item.id} className="p-3 bg-yellow-50 rounded">
                  <div className="flex justify-between items-center mb-1">
        ...
        <div className="border-t pt-4">
          <div className="text-sm text-gray-600">
            {result.successful > 0 && (<div className="text-green-600 font-medium mb-1">
                ✓ {result.successful} facturas procesadas correctamente
              </div>)}
            {result.failed > 0 && (<div className="text-red-600 font-medium mb-1">
                ✗ {result.failed} facturas fallaron en el procesamiento
              </div>)}
            {result.skipped > 0 && (<div className="text-yellow-600 font-medium mb-1">
