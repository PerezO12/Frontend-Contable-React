...
export function JournalEntryProductLine(_a) {
    var line = _a.line, lineIndex = _a.lineIndex, onLineChange = _a.onLineChange, _b = _a.disabled, disabled = _b === void 0 ? false : _b, _c = _a.showProductInfo, showProductInfo = _c === void 0 ? true : _c;
    var _d = useState(null), selectedProduct = _d[0], setSelectedProduct = _d[1];
    var _e = useState(false), showCalculator = _e[0], setShowCalculator = _e[1];
    // Manejar selección de producto
    var handleProductSelect = useCallback(function (product) {
        var _a;
    ...
    var handleProductFieldChange = useCallback(function (field, value) {
    ...
    var updateLineAmountFromProduct = useCallback(function () {
        if (!line.quantity || !line.unit_price)
            return;
        try {
            var quantity = new Decimal(line.quantity);
            var unitPrice = new Decimal(line.unit_price);
            var total = quantity.mul(unitPrice);
            // Aplicar descuentos
            if (line.discount_amount) {
                total = total.minus(new Decimal(line.discount_amount));
            }
            else if (line.discount_percentage) {
                var discountAmount = total.mul(new Decimal(line.discount_percentage)).div(100);
    ...
    var handleToggleCalculator = function () {
    ...
    return (<div className="space-y-3">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
        <div className="flex items-center justify-between mb-2">
          ...
          {selectedProduct && (<div className="flex items-center space-x-2">
              {showProductInfo && (<Button type="button" size="sm" variant="secondary" onClick={handleToggleCalculator} className="text-xs">
                  {showCalculator ? '📊 Ocultar' : '🧮 Calcular'}
                </Button>)}
              <Button type="button" size="sm" variant="danger" onClick={clearProduct} className="text-xs" title="Quitar producto">
                ✕
              </Button>
            </div>)}
        </div>
        
        <ProductSelector value={line.product_id} onSelect={handleProductSelect} onPriceSelect={function (price) {
            if (!line.unit_price || line.unit_price === '0' || line.unit_price === '0.00') {
                handleProductFieldChange('unit_price', price);
            }
        }} disabled={disabled} placeholder="Buscar producto por código o nombre..." className="w-full"/>
      </div>

      {/* Información detallada del producto y calculadora */}
      {selectedProduct && showCalculator && showProductInfo && (<div className="border border-gray-200 rounded-lg">
          <ProductLineInfo product={selectedProduct} quantity={line.quantity || ''} unitPrice={line.unit_price || ''} discountPercentage={line.discount_percentage || ''} discountAmount={line.discount_amount || ''} taxPercentage={line.tax_percentage || ''} taxAmount={line.tax_amount || ''} onQuantityChange={function (value) { return handleProductFieldChange('quantity', value); }} onUnitPriceChange={function (value) { return handleProductFieldChange('unit_price', value); }} onDiscountPercentageChange={function (value) {
            ...
            }} disabled={disabled} showCalculations={true}/>
            {/* Botón para aplicar cálculo al monto de línea */}
          {line.quantity && line.unit_price && (<div className="p-3 bg-gray-50 border-t border-gray-200">
              <Button type="button" onClick={updateLineAmountFromProduct} variant="primary" size="sm" className={"w-full ".concat(parseFloat(line.debit_amount || '0') === 0 && parseFloat(line.credit_amount || '0') === 0
                    ? 'bg-green-600 hover:bg-green-700 animate-pulse'
                    : '')} disabled={disabled}>
                📝 Aplicar Cálculo al Monto de Línea
                {parseFloat(line.debit_amount || '0') === 0 && parseFloat(line.credit_amount || '0') === 0 && (<span className="ml-2">✨</span>)}
              </Button>
      ...
      {selectedProduct && !showCalculator && showProductInfo && (<div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
          <div className="flex items-center justify-between">
            ...
            {line.quantity && line.unit_price && (<div className="text-sm text-gray-600">
      ...
      {selectedProduct && !showProductInfo && (<div className="grid grid-cols-2 gap-2">
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">
              Cantidad
            </label>
            <input type="text" value={line.quantity || ''} onChange={function (e) { return handleProductFieldChange('quantity', e.target.value); }} disabled={disabled} placeholder="0.00" className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"/>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">
              Precio Unit.
            </label>
            <input type="text" value={line.unit_price || ''} onChange={function (e) { return handleProductFieldChange('unit_price', e.target.value); }} disabled={disabled} placeholder="0.00" className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"/>
          </div>
        </div>)}
    </div>);
}
