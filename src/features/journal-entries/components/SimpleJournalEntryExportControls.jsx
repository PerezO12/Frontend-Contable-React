...
export var SimpleJournalEntryExportControls = function (_a) {
    var selectedEntryIds = _a.selectedEntryIds, entryCount = _a.entryCount, onExportStart = _a.onExportStart, onExportEnd = _a.onExportEnd;
    ...
    var handleExport = function () { return __awaiter(void 0, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    console.log('🚀 Iniciando exportación de asientos...', { selectedEntryIds: selectedEntryIds, entryCount: entryCount, exportFormat: exportFormat });
                ...
                case 1:
                    ...
                    return [4 /*yield*/, JournalEntryService.exportJournalEntries(selectedEntryIds, exportFormat)];
                case 2:
                    blob = _a.sent();
                    timestamp = new Date().toISOString().slice(0, 19).replace(/[:-]/g, '');
                    fileName = "asientos_contables_".concat(entryCount, "_registros_").concat(timestamp, ".").concat(exportFormat);
                    console.log('💾 Iniciando descarga del archivo:', fileName);
                    // Descargar archivo
                    ExportService.downloadBlob(blob, fileName);
                    success("\u2705 Se exportaron ".concat(entryCount, " asiento").concat(entryCount === 1 ? '' : 's', " contable").concat(entryCount === 1 ? '' : 's', " exitosamente en formato ").concat(exportFormat.toUpperCase()));
                    return [3 /*break*/, 5];
                case 3:
                    error_1 = _a.sent();
                    console.error('❌ Error al exportar asientos contables:', error_1);
    ...
    return (<div className="flex items-center space-x-3">
      ...
      {/* Botón de exportar */}
      <Button onClick={handleExport} disabled={isExporting || selectedEntryIds.length === 0} variant="primary" className="flex items-center space-x-2">
        {isExporting ? (<>
            <Spinner size="sm"/>
            ...
            <span>Exportar {exportFormat.toUpperCase()}{selectedEntryIds.length > 0 ? " (".concat(entryCount, ")") : ''}</span>
          </>)}
      </Button>
    </div>);
};
