...
export var CashFlowViewer = function (_a) {
    var report = _a.report, _b = _a.className, className = _b === void 0 ? '' : _b;
    var _c = useState('main'), activeSection = _c[0], setActiveSection = _c[1];
    // ==========================================
    // Helper functions
    // ==========================================
    ...
    var getFlowIcon = function (flowValue) {
        var value = parseFloat(flowValue.replace(/,/g, ''));
        if (value > 0)
            return '📈';
        if (value < 0)
            return '📉';
    ...
    var getFlowColor = function (flowValue) {
        var value = parseFloat(flowValue.replace(/,/g, ''));
        if (value > 0)
    ...
    // ==========================================
    var cashFlowMetrics = useMemo(function () {
        var totals = report.table.totals;
        var operatingFlow = parseFloat(totals.flujo_operativo.replace(/,/g, ''));
        var investingFlow = parseFloat(totals.flujo_inversion.replace(/,/g, ''));
        var financingFlow = parseFloat(totals.flujo_financiamiento.replace(/,/g, ''));
        var netFlow = parseFloat(totals.flujo_neto.replace(/,/g, ''));
        return {
    ...
    // Render functions
    // ==========================================
    var renderCashFlowSummary = function () { return (<Card className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <div className="flex items-center gap-4 mt-2">
            <span className={"px-2 py-1 text-xs font-medium rounded-full ".concat(report.table.summary.method === 'direct'
            ? 'bg-blue-100 text-blue-800'
            : 'bg-green-100 text-green-800')}>
              Método {report.table.summary.method === 'direct' ? 'Directo' : 'Indirecto'}
            </span>
            {report.table.summary.is_reconciled && (<span className="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">
                ✅ Reconciliado
              </span>)}
          </div>
        </div>
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={"text-xl font-bold ".concat(getFlowColor(report.table.totals.flujo_operativo))}>
        ...
        <div className="bg-orange-50 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={"text-xl font-bold ".concat(getFlowColor(report.table.totals.flujo_inversion))}>
        ...
        <div className="bg-purple-50 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={"text-xl font-bold ".concat(getFlowColor(report.table.totals.flujo_financiamiento))}>
        ...
        <div className="bg-gray-50 p-4 rounded-lg border-2 border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={"text-xl font-bold ".concat(getFlowColor(report.table.totals.flujo_neto))}>
      ...
      {report.table.summary.beginning_cash_balance && report.table.summary.ending_cash_balance && (<div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gradient-to-r from-gray-50 to-blue-50 rounded-lg">
          ...
          <div className="text-center">
            ...
            <p className={"text-lg font-bold ".concat(getFlowColor(report.table.totals.flujo_neto))}>
    ...
    var renderDetailedTable = function () { return (<Card className="p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">
        📋 Detalle por Actividades
      </h3>

      <div className="space-y-6">
        {report.table.sections.map(function (section, index) { return (<div key={index} className="border rounded-lg overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {section.items.map(function (item, itemIndex) { return (<tr key={itemIndex} className={"".concat(item.level > 1 ? 'bg-gray-50' : '')}>
                      <td className={"px-4 py-2 text-sm ".concat(item.level > 1 ? 'pl-8' : '')}>
                        <div>
                          <div className="font-medium text-gray-900">
                            {item.account_name}
                          </div>
                          {item.account_code && (<div className="text-xs text-gray-500">
                              {item.account_code}
                            </div>)}
                        </div>
                      </td>
                      ...
                      <td className={"px-4 py-2 text-sm text-right font-mono font-medium ".concat(getFlowColor(item.movements))}>
    ...
    // Main render
    // ==========================================
    return (<div className={"space-y-6 ".concat(className)}>      {/* Navigation tabs */}
      <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg">
        <button onClick={function () { return setActiveSection('main'); }} className={"px-4 py-2 rounded-md text-sm font-medium transition-colors ".concat(activeSection === 'main'
            ? 'bg-white text-blue-600 shadow-sm'
            : 'text-gray-500 hover:text-gray-700')}>
          💧 Flujo de Efectivo
        </button>
        <button onClick={function () { return setActiveSection('import_export'); }} className={"px-4 py-2 rounded-md text-sm font-medium transition-colors ".concat(activeSection === 'import_export'
            ? 'bg-white text-blue-600 shadow-sm'
            : 'text-gray-500 hover:text-gray-700')}>
          📥 Importar
        </button>
      ...
      {activeSection === 'import_export' && (<div className="space-y-6">
          <Card className="p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              📤 Exportar Datos
            </h3>
