...
export var ThirdPartyExportModal = function (_a) {
    var thirdParties = _a.thirdParties, isOpen = _a.isOpen, onClose = _a.onClose, _b = _a.title, title = _b === void 0 ? 'Exportar Terceros' : _b;
    ...
    var filteredThirdParties = useMemo(function () {
        return thirdParties.filter(function (thirdParty) {
            var _a, _b;
            var matchesSearch = searchTerm === '' ||
                ((_a = thirdParty.code) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(searchTerm.toLowerCase())) ||
                thirdParty.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                ((_b = thirdParty.commercial_name) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes(searchTerm.toLowerCase())) ||
            ...
            var matchesTypeFilter = typeFilter === '' || thirdParty.third_party_type === typeFilter;
    ...
    var selectedStats = useMemo(function () {
        var selected = thirdParties.filter(function (thirdParty) { return selectedThirdParties.has(thirdParty.id); });
        var byType = selected.reduce(function (acc, thirdParty) {
            acc[thirdParty.third_party_type] = (acc[thirdParty.third_party_type] || 0) + 1;
            return acc;
        }, {});
        return {
            total: selected.length,
            active: selected.filter(function (tp) { return tp.is_active; }).length,
            inactive: selected.filter(function (tp) { return !tp.is_active; }).length,
            byType: byType
        };
    }, [thirdParties, selectedThirdParties]);
    // Manejar selección individual
    var handleThirdPartySelect = useCallback(function (thirdPartyId, checked) {
    ...
    var handleSelectAll = useCallback(function (checked) {
        if (checked) {
            var allFilteredIds = new Set(__spreadArray(__spreadArray([], selectedThirdParties, true), filteredThirdParties.map(function (thirdParty) { return thirdParty.id; }), true));
            setSelectedThirdParties(allFilteredIds);
        }
        else {
            var filteredIds_1 = new Set(filteredThirdParties.map(function (thirdParty) { return thirdParty.id; }));
            var newSelected = new Set(__spreadArray([], selectedThirdParties, true).filter(function (id) { return !filteredIds_1.has(id); }));
            setSelectedThirdParties(newSelected);
        }
    ...
    var handleSelectActiveOnly = useCallback(function () {
        var activeIds = new Set(thirdParties.filter(function (thirdParty) { return thirdParty.is_active; }).map(function (thirdParty) { return thirdParty.id; }));
        setSelectedThirdParties(activeIds);
        setSelectAll(false);
        success("\u2705 Seleccionados ".concat(activeIds.size, " terceros activos"));
    }, [thirdParties, success]);
    // Seleccionar por tipo de tercero
    var handleSelectByType = useCallback(function (thirdPartyType) {
        var typeIds = new Set(thirdParties.filter(function (thirdParty) { return thirdParty.third_party_type === thirdPartyType; }).map(function (thirdParty) { return thirdParty.id; }));
        setSelectedThirdParties(typeIds);
        setSelectAll(false);
        success("\u2705 Seleccionados ".concat(typeIds.size, " terceros de tipo ").concat(THIRD_PARTY_TYPE_LABELS[thirdPartyType]));
    }, [thirdParties, success]);
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    selectedIds = Array.from(selectedThirdParties);
                    return [4 /*yield*/, ThirdPartyService.exportThirdParties(selectedIds, exportFormat)];
                case 2:
                    blob = _a.sent();
                    timestamp = new Date().toISOString().slice(0, 19).replace(/[:-]/g, '');
                    fileName = "terceros_".concat(selectedThirdParties.size, "_registros_").concat(timestamp, ".").concat(exportFormat);
                    // Descargar archivo
                    ExportService.downloadBlob(blob, fileName);
                    success("\u2705 Se exportaron ".concat(selectedThirdParties.size, " tercero").concat(selectedThirdParties.size === 1 ? '' : 's', " exitosamente en formato ").concat(exportFormat.toUpperCase()));
                    onClose();
                    return [3 /*break*/, 5];
                case 3:
                    error_1 = _a.sent();
                    console.error('Error al exportar terceros:', error_1);
      ...
      <div className="relative z-[999999] h-full flex items-center justify-center p-6">
        ...
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col" onClick={function (e) { return e.stopPropagation(); }}>
          ...
          <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-green-600 to-green-700 text-white rounded-t-xl">
            ...
            <button onClick={handleClose} disabled={isExporting} className="text-green-100 hover:text-white hover:bg-white/20 p-2 rounded-lg transition-colors">
          ...
          <div className="flex-1 min-h-0 flex flex-col">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="md:col-span-2">
                    <Input type="text" placeholder="Buscar por código, nombre, razón social o documento..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} disabled={isExporting} className="w-full"/>
                  </div>
                  <div>
                    <select value={activeFilter} onChange={function (e) { return setActiveFilter(e.target.value); }} disabled={isExporting} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500">
                  ...
                  <div>
                    <select value={typeFilter} onChange={function (e) { return setTypeFilter(e.target.value); }} disabled={isExporting} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500">
            ...
            {/* Estadísticas */}
            {selectedThirdParties.size > 0 && (<div className="px-6 py-3 bg-green-50 border-b flex-shrink-0">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <label className="flex items-center gap-2">
                    <input type="checkbox" checked={selectAll && filteredThirdParties.length > 0} onChange={function (e) { return handleSelectAll(e.target.checked); }} disabled={isExporting || filteredThirdParties.length === 0} className="rounded border-gray-300 text-green-600 focus:ring-green-500"/>                    
                    <span className="text-sm font-medium">
                      {selectAll && filteredThirdParties.length > 0 ? "".concat(selectedThirdParties.size, " seleccionados") : "Seleccionar todos"}
                    </span>
                  </label>
                </div>
              </div>
            ...
            var count = thirdParties.filter(function (tp) { return tp.third_party_type === type; }).length;
            if (count === 0)
                return null;
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="p-6">
                {filteredThirdParties.length === 0 ? (<div className="text-center py-12">
                    ...
                    {filteredThirdParties.map(function (thirdParty) { return (<label key={thirdParty.id} className={"flex items-start gap-3 p-4 border-2 rounded-lg cursor-pointer transition-all hover:shadow-sm ".concat(selectedThirdParties.has(thirdParty.id)
                    ? 'border-green-300 bg-green-50'
                    : 'border-gray-200 hover:border-gray-300')}>
                        <input type="checkbox" checked={selectedThirdParties.has(thirdParty.id)} onChange={function (e) { return handleThirdPartySelect(thirdParty.id, e.target.checked); }} disabled={isExporting} className="mt-1 rounded border-gray-300 text-green-600 focus:ring-green-500"/>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            {thirdParty.code && (<code className="text-sm font-mono bg-gray-100 px-2 py-1 rounded text-green-600 font-semibold">
                                {thirdParty.code}
                              </code>)}
                            <span className={"text-xs px-2 py-1 rounded-full font-medium ".concat(thirdParty.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600')}>
                          ...
                          {thirdParty.commercial_name && thirdParty.commercial_name !== thirdParty.name && (<div className="text-sm text-gray-600 mb-1">
                          ...
                          {thirdParty.email && (<p className="text-sm text-gray-500 mt-1 truncate">
                              {thirdParty.email}
                            </p>)}
                        </div>
                      </label>); })}
          ...
          <div className="flex items-center justify-between p-6 border-t bg-gray-50 rounded-b-xl flex-shrink-0">
            <div className="flex items-center gap-4">
              <span className="text-sm text-gray-600">
                {selectedThirdParties.size > 0 ? (<span className="font-medium text-green-600">
                    {selectedThirdParties.size} tercero{selectedThirdParties.size === 1 ? '' : 's'} seleccionado{selectedThirdParties.size === 1 ? '' : 's'}
                  </span>) : ('Selecciona al menos un tercero para continuar')}
              </span>
              {selectedThirdParties.size > 0 && (<Button variant="ghost" size="sm" onClick={handleClearSelection} disabled={isExporting}>
                  Limpiar selección
                </Button>)}
            </div>
            <div className="flex gap-3">
              ...
              <Button onClick={handleExport} disabled={selectedThirdParties.size === 0 || isExporting} className="min-w-[120px]">
